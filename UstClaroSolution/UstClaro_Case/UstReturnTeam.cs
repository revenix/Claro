...
namespace UstClaro_Case
{
    ...
    public class UstReturnTeam : IPlugin
    {
            ...
            try
            {
                if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
                {
                    if (context.MessageName == "Update")
                    {
                        ...
                        var fetchXmlPoint = "";
                        var fetchXmlTeam = "";
                        EntityReference teamlocallaboratory = null;
                        EntityReference teamcentrallaboratory = null;

                        Entity target = (Entity)context.InputParameters["Target"];

                        if (target.Attributes.Contains("ust_flagreturnteamplugin") && target["ust_flagreturnteamplugin"] != null)
                        {
                            ...
                            if (ostPresential != null)
                            {
                                ...
                                if (originpos != null)
                                {
                                    if (diagnosticlab == 864340000) //Local
                                    {
                                        ...
                                        if (resultP[0].Attributes.Contains("ust_teamlocallaboratory") && resultP[0]["ust_teamlocallaboratory"] != null)
                                        {
                                            teamlocallaboratory = ((EntityReference)resultP[0].Attributes["ust_teamlocallaboratory"]);
                                            //myTrace.Trace("ust_teamlocallaboratory :" + teamadvisorspdv.Name);
                                        }
                                        ...
                                        fetchXmlTeam = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>" +
                                                       "<entity name='team'>" +
                                                        ...
                                                        "<filter type='and'>" +
                                                        "<condition attribute='name' operator='eq' value='" + teamlocallaboratory.Name + "' />" +
                                                        "</filter>" +
                                                        "</entity>" +
                                                        "</fetch>";
